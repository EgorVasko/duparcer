...
def all():
    ...
    #dictold = functions.openbase("data/lxml_data.json", "Old")
    #dictred = functions.openbase("data/lxml_data_disc.json", "Old discounted")
    #dictsold = functions.openbase("data/lxml_data_sold.json", "Sold")

    # testing database threading
    bases = [["newcars", dictold], ["soldcars", dictsold], ["discounted", dictred]]
    threads = []
    for base in bases:
        t = threading.Thread(target=database.read_from_database, args=base)
        t.start()
        threads.append(t)
    for base in threads:
        base.join()
    ...
    functions.combine(parsed, dictred, dict_disc_again, dict_red_nochanges)
    functions.combine(parsed, dictold, dict_disc, dict_nochanges)
    dict_new = parsed
    ...
    for dict_ in dicttosort:
        functions.sort_by_parametr(dict_, sort)

    # load_to_html()

    output_in_var = functions.load_to_variable(dict_new, dict_disc_again, dict_disc, dict_nochanges, dict_red_nochanges, out_sold, time_of_parse)

    #functions.dump_data(dictold, out_new, "data/lxml_data.json")
    #functions.dump_data(dictred, out_disc, "data/lxml_data_disc.json")
    #functions.dump_data(dictsold, out_sold, "data/lxml_data_sold.json")

    bases = [dictold, out_new, "newcars"], [dictred, out_disc, "discounted"], [dictsold, out_sold, "soldcars"]
    threads = []
    for base in bases:
        t = threading.Thread(target=database.dump_to_base, args=base)
        t.start()
        threads.append(t)
    for base in threads:
        base.join()
    ...
    functions.exitmessage(result, dict_new)
    return output_in_var
